...
.container
{
  ...
  width: 98%;
      ...
      <?php
    $sql="SELECT * FROM student INNER JOIN marksheet ON student.Reg_no = marksheet.Reg_no WHERE stream_id IN (SELECT stream_id FROM stream NATURAL JOIN class WHERE c_name='".$form."') AND term_id='".$_SESSION['term_id']."' AND stud_id='".$stud_id."' AND year='".$_SESSION['year']."' ORDER BY total DESC";
          $runst    =mysqli_query($conn,$sql) or die(mysqli_error($conn));

      $sqlT="SELECT * FROM student INNER JOIN marksheet ON student.Reg_no = marksheet.Reg_no WHERE stream_id IN (SELECT stream_id FROM stream NATURAL JOIN class WHERE c_name='".$form."') AND term_id='".$_SESSION['term_id']."' AND year='".$_SESSION['year']."' ORDER BY total DESC";
          $runstT    =mysqli_query($conn,$sqlT) or die(mysqli_error($conn));

          $total_students =mysqli_num_rows($runstT);
        ...
        while($result=mysqli_fetch_assoc($runst))
      {
          ...
          //generate stream
            $stream_q ="SELECT stream,c_name FROM stream NATURAL JOIN class WHERE stream_id='".$stream_id."'";
            $result_stream    =mysqli_query($conn,$stream_q);
            $rows =mysqli_fetch_array($result_stream);
            $stream =$rows['c_name'].' '.$rows['stream'];
   ...
   <div class="container" style=" border: 1px solid black; border-radius: 10px; padding: 10px; " id="PrintDivd">
      ...
      <div width="100%">
           <table width="98%" cellpadding="0" cellspacing="0" style="border: none;" >
             ...
             <tr style="border: none; padding: 5px;">
                ...
                <td style="border: none; padding: 5px;"><p align="right">OUT OF:<?php echo $total_students; ?></p></td>
